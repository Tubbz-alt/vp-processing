...
```{r 1_rmd_settings, include=FALSE}
...
```{r 1_libraries, results="hide", warning=FALSE, message=FALSE}
source("functions/load_settings.R")
# source("functions/load_radars.R")
```

## Download code

To run or adapt the code in this document, fork or clone this repository: https://github.com/enram/timamp-etl.

## Define processing settings {#settings}

The processing requires a number of settings we need to define. If you want to [run the processing with other data and settings](#own-processing), then **this is the only code you need to update** (in `1_setup.Rmd`):

```{r 1_define_settings}
# The following three paths are relative to this script (i.e. to the src directory)
raw_data_dir <- "../data/raw/example/" # Needs a trailing slash!
# settings_file <- "../settings/example_settings.yaml"
# radars_file <- "../settings/example_radars.csv"
```

Load those settings:

```{r 1_load_settings, results="hide", warning=FALSE, message=FALSE}
# settings <- load_settings(settings_file)
# radars <- load_radars(radars_file)
```

## Run processing

The processing of the vp data is done in four steps. Each has its own chapter and [`Rmd file`](https://github.com/enram/timamp-etl/tree/master/src):

1. [Get started](setup.html) (`1_setup.Rmd`) = this chapter
3. [Extract vp data](extract-vp.html) (`2_extract_vp.Rmd`)
4. [Process vp data for the Bird migration flow visualization](vp-to-flowviz.html) (`3_vp_to_flowviz.Rmd`)
5. [Process vp data for TIMAMP](vp-to-timamp.html) (`4_vp_to_timamp.Rmd`)

The repository comes with [example settings](https://github.com/enram/timamp-etl/tree/master/settings) and [data](https://github.com/enram/timamp-etl/tree/master/data) for all steps, so you can run the processing out of the box:

1. Go the `src` directory of the downloaded code
2. Open `timamp-etl.Rproj` in RStudio
...
If you are adapting the code, it can be useful to run individual steps. Except for `1_setup.Rmd`, all steps read input data file(s) and generate output data file(s). If those data files are already there, you don't need to rerun the step that generated those. However, all files use the [processing settings](#settings) defined in `1_setup.Rmd`, so you need to run that file at least once first:

1. Open and run `1_setup.Rmd` in RStudio
...
If the processing doesn't fit your needs, you can update the code by editing the relevant file in `src` and running it (see \@ref(run-individual-steps)). If you encounter a bug in the original code, please let us know by [filing an issue](https://github.com/enram/timamp-etl/issues/new) or submitting a pull request.
