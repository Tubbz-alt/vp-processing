# Extract, aggregate and export vertical profile data {#extract_vp}
...
```{r 2_vp_head, echo=FALSE}
head(vp_df)
```

Number of records:

```{r 2_vp_nrow, echo=FALSE}
nrow(vp_df)
```
...
```{r 2_vp_add_bins_head, echo=FALSE}
vp_df
```
