...
```{r}
vp.data <- subset(vp.one$data, select = c("HGHT", "u", "v", "dens"))
vp.data
```
...
get_metadata <- function(vp) {
  ...
  # Retrieve and combine date and time
  datetime_string <- paste0(vp$attributes$what$date, vp$attributes$what$time)
  # Express as ISO datetime
  metadata["datetime"] <- format(as.POSIXct(datetime_string, format = "%Y%m%d%H%M%S"), "%Y-%m-%dT%H:%M:%SZ")
  
  # Split source information on comma
  source <- unlist(strsplit(vp$attributes$what$source, ","))
  # Get radar_id part (e.g. "seang") from first source element (e.g. "NOD:seang"):
  metadata["radar_id"] <- unlist(strsplit(source[1], ":"))[2]
