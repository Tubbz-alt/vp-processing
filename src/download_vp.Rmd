# Download vertical profile data from the ENRAM data repository {#download_vp}

```{r 2_setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

The steps described in this chapter can be skipped if all required vertical profile data are already available locally. We will need the bioRad package for the next steps too though, so make sure it is installed.

## Install bioRad

Follow step 1 to 3 of the [bioRad installation instructions](https://github.com/adokter/bioRad#installation):

```{r 2_install_biorad, eval=FALSE}
library(devtools)
...
## Download vertical profile data

Download the data. Note that this step might take some time:

```{r 2_download_data, results="hide", eval=FALSE}
bioRad::download_vp(
...
We now have all the data stored locally. 👏 In the [next chapter](extract-vp.html) we'll export what we need for the visualization in a CSV file.
