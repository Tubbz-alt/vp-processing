...
```{r extract_libraries, results="hide", warning=FALSE, message=FALSE}
library(bioRad)
library(dplyr)
library(lubridate)
source("vp_to_df.R")
...
```{r extract_file_paths_add_data_dir}
...
```{r extract_vp_read_all_with_biorad}
...
```{r extract_vp_head, echo=FALSE}
head(vp_df)
```

Number of records:

```{r extract_vp_nrow, echo=FALSE}
nrow(vp_df)
```
...
```{r extract_vp_filter_nrow, echo=FALSE}
nrow(vp_df)
```
...
```{r extract_vp_add_bins_head, echo=FALSE}
vp_df
```
...
```{r extract_vp_agg_head, echo=FALSE}
head(vp_agg_df)
```

## Export aggregated vp data to a CSV file

```{r extract_vp_agg_export}
