...
vp_file_paths <- bioRad::retrieve_vp_paths(
  ...
  # Since 3 letter radar codes are not always unique across countries, 
  # we select on countries as well
  country = settings$general$countries,
  path = raw_data_dir
)
```
...
```{r extract_vp_export}
vp_data %>% select(-id) -> vp_data # Remove id column
write.csv(vp_data, file = paste0(processed_data_dir, "vp_data.csv"), na = "", row.names = FALSE)
```
