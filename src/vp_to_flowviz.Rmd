...
To allow this chapter to run without having to process all the vp data, we'll load the data from the CSV file rather than from the dataframe created in \@ref(extract-vp):

```{r flowviz_processed_data_file}
processed_data_file = paste0(processed_data_dir, settings$general$vp_output_file)
processed_data_file
```

```{r flowviz_read_data}
vp_data <- read.csv(processed_data_file)
head(vp_data)
```
...
vp_data %>%
  mutate(datetime_bin = floor_date(as.POSIXct(datetime, tz = "UTC"), "hour")) -> vp_data
```
...
```{r}
write.csv(vp_data_flowviz, file = paste0(processed_data_dir, "flowviz/birds.csv"), na = "", row.names = FALSE)
```

## Create metadata

Not yet implemented, currently created manually.
